...
const getDirModules = (path, option) => {
    ...
    let filePathArr = getDirFilesPath_1.getDirFilesPath(path, { deep });
    const result = [];
    filePathArr.filter((filePath) => includes.some((reg) => reg.test(filePath)));
    if (excludes.length) {
        //匹配到的则移除
        filePathArr = filePathArr.filter((filePath) => excludes.some((reg) => reg.test(path_1.basename(filePath))) === false);
    }
    //根据文件名去读取module，
